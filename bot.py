...
async def show_sub(target, context, channels, lang):
    kb = [[InlineKeyboardButton(f"📢 {ch['channel_name']}", url=ch["channel_url"])] for ch in channels]
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not_subbed:
        await show_sub(update, context, not_subbed, lang)
        return
    await update.message.reply_text("👇", reply_markup=bottom_kb())
    await update.message.reply_text(t(lang, "start_msg"), reply_markup=main_menu_kb(lang, is_admin(user.id)), parse_mode="Markdown")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith("set_lang_"):
        ...
        if not_subbed:
            await show_sub(query, context, not_subbed, lang)
            return
        await query.message.reply_text("👇", reply_markup=bottom_kb())
        await query.edit_message_text(t(lang, "start_msg"), reply_markup=main_menu_kb(lang, is_admin(uid)), parse_mode="Markdown")
        return
    ...
    if d == "contact_admin":
        ...
        await query.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(t(lang, "cancel"), callback_data="menu")]])
        )
        return
...
async def admin_cb(query, context, uid, d, lang):
    if d == "admin_panel":
    ...
    elif d.startswith("adm_ep_") or d.startswith("adm_add_ep_"):
        ...
        await query.edit_message_text(f"📺 *{movie['title']}*\n\nVideo yuboring:", reply_markup=cancel_kb(), parse_mode="Markdown")
        ...
        text = "📋 Hozircha kanal yo'q." if not subs else "📋 *Kanallar:*\n\n" + "".join(f"{i}. {s['channel_name']} | `{s['channel_id']}`\n" for i, s in enumerate(subs, 1))
        await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Orqaga", callback_data="adm_subs")]]), parse_mode="Markdown")
        ...
        await query.edit_message_text("📝 Kanal ID yoki @username yozing:",
        ...
        await query.edit_message_text("💎 VIP bermoqchi bo'lgan foydalanuvchi ID sini yozing:", reply_markup=cancel_kb())

    elif d.startswith("vip_give_"):
        parts = d.split("_")
        target_id, days = int(parts[2]), int(parts[3])
        until = set_vip(target_id, days)
        try:
            await context.bot.send_message(chat_id=target_id, text=f"🎉 Sizga {days} kunlik VIP berildi!\n📅 Muddat: *{until}*", parse_mode="Markdown")
        except:
            pass
        await query.edit_message_text(f"✅ {target_id} ga {days} kunlik VIP.\nMuddati: {until}",
        ...
        msgs = get_messages()
        if not msgs:
            await query.edit_message_text("📩 Hozircha xabar yo'q.",
                                           reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Orqaga", callback_data="admin_panel")]]))
            return
        text = "📩 *Xabarlar:*\n\n"
        kb = []
        ...
        await query.message.reply_text(f"✏️ `{target_id}` ga javob yozing:", reply_markup=cancel_kb(), parse_mode="Markdown")
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🏠 Bosh menyu":
        ...
        if not_subbed:
            await show_sub(update, context, not_subbed, lang)
            return
        await update.message.reply_text(t(lang, "start_msg"), reply_markup=main_menu_kb(lang, is_admin(uid)), parse_mode="Markdown")
        return
        ...
        if code.isdigit():
            ...
            if movie:
                ...
                if eps:
                    try:
                        user_states.pop(uid, None)
                        await context.bot.send_video(chat_id=uid, video=eps[0]["file_id"],
                                                      caption=f"🎬 *{movie['title']}*",
                                                      protect_content=protect, parse_mode="Markdown")
                    except:
                ...
                await update.message.reply_text("❌ Bu kodda kino topilmadi! Qaytadan kiriting:")
        else:
        ...
        for admin_id in ADMIN_IDS:
            try:
                await context.bot.send_message(chat_id=admin_id,
                    ...
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("↩️ Javob berish", callback_data=f"reply_to_{uid}")]]),
                    parse_mode="Markdown")
            except:
                pass
    ...
    if step == "movie_title":
        ...
        if text.isdigit():
            ...
            if movie:
                ...
                await update.message.reply_text(f"✅ {movie['title']} o'chirildi!",
                                                 reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Admin panel", callback_data="admin_panel")]]))
            else:
        ...
        admin_states.pop(uid, None)
        await update.message.reply_text(f"✅ {state['channel_name']} qo'shildi!",
        ...
        if text.isdigit():
            ...
            await update.message.reply_text(f"✅ {text} admin qilindi!",
                                             reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Admin panel", callback_data="admin_panel")]]))
        else:
        ...
        admin_states.pop(uid, None)
        try:
            await context.bot.send_message(chat_id=target_id, text=f"📬 *Admin javob berdi:*\n\n{text}", parse_mode="Markdown")
...
async def payment_success(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if plan:
        ...
        await update.message.reply_text(f"🎉 VIP faollashdi!\n📅 Tugash: {until}", reply_markup=back_kb(lang))

async def join_request_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        await context.bot.approve_chat_join_request(chat_id=update.chat_join_request.chat.id, user_id=update.chat_join_request.from_user.id)
    except:
        pass
...
def main():
    ...
    app.add_handler(MessageHandler((filters.VIDEO | filters.PHOTO | filters.Document.ALL) & ~filters.COMMAND, media_handler))
